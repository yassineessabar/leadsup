...
// Helper function to enroll customer in campaigns based on request type
async function enrollCustomerInCampaigns(userId: string, customer: any, requestType: string) {
  try {
    // Call the campaign enrollment API
    ...
    const enrollmentResponse = await fetch(`${baseUrl}/api/campaigns/enroll-contact`, {
      ...
      headers: {
        ...
        'Cookie': `session=${await getSessionToken()}` // Pass session for auth
      },
      body: JSON.stringify({
    ...
    if (enrollmentResponse.ok) {
      ...
      console.log(`🎯 Campaign enrollment result for ${customer.name}:`, result.message)
      return result
    } else {
      console.error(`Failed to enroll ${customer.name} in campaigns:`, await enrollmentResponse.text())
    }
...
// Helper to get session token for internal API calls
...
<body style="margin: 0; padding: 0; background-color: #f8fafc;">
  <div class="container" style="max-width: 650px; margin: 0 auto; background-color: white; font-family: 'Inter', sans-serif;">
    ...
    // Auto-enroll customer in campaigns based on request type
    if (requestStatus === "sent") {
      await enrollCustomerInCampaigns(userId, customer, request_type)
    }
