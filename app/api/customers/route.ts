...
async function triggerAutomationForNewCustomer(userId: string, customer: any) {
  ...
  // Also trigger campaign enrollment for the new customer
  try {
    await enrollCustomerInCampaigns(userId, customer)
  } catch (campaignError) {
    console.error('Error enrolling customer in campaigns:', campaignError)
    // Don't fail if campaign enrollment fails
  }
}

// Helper function to enroll new customer in campaigns based on their communication type preference
async function enrollCustomerInCampaigns(userId: string, customer: any) {
  try {
    // Get the user's session token for internal API calls
    ...
    if (sessionError || !sessionData?.session_token) {
      console.log(`⚠️ No session token found for user ${userId}, skipping campaign enrollment`)
      return
    }
    ...
    // Enroll in email campaigns if customer prefers email communication and has an email
    if ((customer.type === 'email' || customer.type === 'both') && customer.email) {
      const enrollmentResponse = await fetch(`${baseUrl}/api/campaigns/enroll-contact`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (enrollmentResponse.ok) {
        ...
        console.log(`📧 Email campaign enrollment for ${customer.name} (type: ${customer.type}):`, result.message)
        if (result.enrollments && result.enrollments.length > 0) {
          const successfulEnrollments = result.enrollments.filter(e => e.status === 'enrolled')
          console.log(`   ✅ Successfully enrolled in ${successfulEnrollments.length} email campaigns`)
        }
      } else {
        console.error(`Failed email enrollment for ${customer.name}:`, await enrollmentResponse.text())
      }
    }

    // Enroll in SMS campaigns if customer prefers SMS communication and has a phone
    if ((customer.type === 'sms' || customer.type === 'both') && customer.phone) {
      const enrollmentResponse = await fetch(`${baseUrl}/api/campaigns/enroll-contact`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (enrollmentResponse.ok) {
        const result = await enrollmentResponse.json()
        console.log(`📱 SMS campaign enrollment for ${customer.name} (type: ${customer.type}):`, result.message)
        if (result.enrollments && result.enrollments.length > 0) {
          const successfulEnrollments = result.enrollments.filter(e => e.status === 'enrolled')
          console.log(`   ✅ Successfully enrolled in ${successfulEnrollments.length} SMS campaigns`)
        }
      } else {
        console.error(`Failed SMS enrollment for ${customer.name}:`, await enrollmentResponse.text())
      }
    }

    console.log(`🎯 Campaign enrollment completed for ${customer.name} with communication type: ${customer.type}`)

  } catch (error) {
    console.error('Error in enrollCustomerInCampaigns:', error)
  }
}
