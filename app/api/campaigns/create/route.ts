  ...
  "value_proposition": {
    ...
    const sequenceResponse = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are an expert email marketing copywriter. Generate 6 compelling emails for a 2-sequence campaign based on the company, ICP, and value proposition. First 3 emails are initial outreach, last 3 are re-engagement after 60 days. Write in ${formData.language || 'English'}. Return only valid JSON without code blocks or markdown.`
        },
        {
          role: "user",
