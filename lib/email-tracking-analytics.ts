...
export async function getEmailTrackingMetrics(
  userId: string, 
  startDate: string, 
  endDate: string
): Promise<EmailTrackingMetrics | null> {
  try {
    console.log(`📊 Fetching email tracking metrics for user ${userId} from ${startDate} to ${endDate}`)
    
    // Get all email tracking records for the user in the date range
    ...
    if (trackingError) {
      console.error('❌ Error fetching email tracking records:', trackingError)
      return null
    }
    
    if (!trackingRecords || trackingRecords.length === 0) {
      console.log('⚠️ No email tracking records found for this period')
      return {
        emailsSent: 0,
        emailsDelivered: 0,
        emailsBounced: 0,
    ...
    console.log(`📧 Found ${trackingRecords.length} email tracking records`)
    ...
    console.log('📊 Email tracking metrics calculated:', {
    ...
    console.error('❌ Error calculating email tracking metrics:', error)
    return null
  }
}